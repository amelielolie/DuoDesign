...
const BILL_POSITIONS = Array.from({ length: BILL_COUNT }, (_, i) => {
  ...
  return {
    ...
    y: isHigh
      ? 40 + seededRandom(i, 100) * 4   // high: 40-44%, need jump
...
const FROZEN_BILLS = Array.from({ length: 8 }, (_, i) => ({
  ...
  y: i % 2 === 0
    ? 40 + seededRandom(i, 200) * 4     // high: 40-44%, need jump
...
export function GameWorld() {
  ...
  const animate = useCallback((timestamp: number) => {
    ...
    if (ms > 0) {
      ...
      const charHeightPct = charDivHeightPct * 0.72  // visible body including head
